...
async function assertSection(sectionId: string, examId?: string) {
  ...
  if (examId && String(section.examId) !== String(examId)) {
    throw new ApiError(400, 'Section does not belong to the selected exam', 'INVALID_SECTION_RELATIONSHIP');
  }
  return section;
}
...
function assertOptions(options: Array<{ key: string; text: string }>, correctOptions: string[]) {
  ...
  if (correctOptions.some(key => !keys.includes(key))) {
    throw new ApiError(400, 'Correct answers must reference valid option keys', 'INVALID_QUESTION_OPTIONS');
  }
}
...
export async function listQuestions(query: Record<string, unknown>) {
  ...
  if (query.examId || query.exam) {
    const examId = String(query.examId ?? query.exam);
    const sections = await Section.find({ examId }).select('_id').lean();
...
export async function getQuestion(id: string, includeInactive = false) {
  const question = await Question.findOne({ _id: id, ...(includeInactive ? {} : { isActive: true }) }).select('-correctOptions').lean();
...
export async function updateQuestion(id: string, input: Record<string, any>) {
  ...
  const section = await assertSection(String(sectionId), input.examId);
